...
do
	iam=`hostname -s`
	mkdir ./running/$iam/ 2>/dev/null
	loc=`ls -1 ./pending/$iam/*.sh 2>/dev/null | wc -l`
	if [ $loc != 0 ]; then
			file=`ls ./pending/$iam/*.sh | head -n 1`
			echo $file
			if [ ! -z $file ]; then
				if [ -f $file ]; then
...
# loop through selected ROPs    
	...
	for i in ops :
		...
		if otn == 'rop_geometry' or otn == 'rop_dop' or otn == 'geometry' or otn == 'wedge' or otn == 'rop_comp' or otn == 'comp' or otn == 'ifd' or otn == 'opengl' or otn == 'prepost':
			opn = i.name()
			opth = i.path()
...
# is the wedge driver a sim?                    
				...
				try :
					if wd.parm('vm_tile_render').eval() == True :
						wisasim = False
				except :
					print("wedge driver has no tile parameter")
...
# set wedge driver output manually as there is no wedgeval yet                
					...
					if wtn == 'ifd' : 
						if wd.parm('vm_tile_render').eval() == True :
							wd.parm('vm_picture').set("$HIP/pic/${OS}/${OS}.pic")
						else :
							wd.parm('vm_picture').set("$HIP/pic/${OS}_" + str(cc) + "/${OS}_" + str(cc) + ".$F4.pic")
					...
					if wtn == 'comp' : wd.parm('copoutput').set("$HIP/out/${OS}_" + str(cc) + "/${OS}_" + str(cc) + ".$F4." + ext)
					if wtn == 'rop_comp' : wd.parm('copoutput').set("$HIP/out/${OS}_" + str(cc) + "/${OS}_" + str(cc) + ".$F4." + ext)
...
# create batch scripts per frame for non-sims:
					...
					if not wisasim :
						...
						for s in range(int(sfdur)) :
							...
							if batchcmd != "" :
								nn = datetime.strftime(datetime.now(),'%y%m%d_%H%M%S%f')
								#print(batchcmd)
...
# reset the wedge driver file fields to my defaults
					...
					if wtn == 'ifd' : wd.parm('vm_picture').set("$HIP/pic/${WEDGENUM}_${OS}/${WEDGENUM}_${OS}.$F4.pic")
					...
					if wtn == 'comp' : wd.parm('copoutput').set("$HIP/out/${WEDGENUM}_${OS}/${WEDGENUM}_${OS}.$F4." + ext)
					if wtn == 'rop_comp' : wd.parm('copoutput').set("$HIP/out/${WEDGENUM}_${OS}/${WEDGENUM}_${OS}.$F4." + ext)
...
# its a utility ROP, just send it, see what happens...
