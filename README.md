...
note:
 ...
 To get multiple machines on it just run themo.sh on the other computers 
 (assuming they can see the farm dir).
 
 Composite rops are sent as 10s (cops too leaky to go higher).
 Compisite jobs may not successfully overwrite existing frames! I've reported this as a bug.  
 Work around it by deleting existing frames 1st.
 ...
 Each machine will prefer scripts in a directory matching its name, 
 eg: machine skullet will grab jobs from /farm/pending/skullet 1st.
 Whitelist by adding a 'whitelist' string parameter to the rop(s), 
 separate machine names with spaces.
